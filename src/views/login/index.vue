<template>
  <div class="login-container">
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" autocomplete="on" label-position="left">
      ...
      <el-button type="text" style="margin-bottom:30px;margin-left: 0px;" @click="$router.push('/register')">Sign Up</el-button>
...
import { login, userInfo, loginActive } from '@/apis'
  ...
  methods: {
    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          this.loading = true
          login(this.loginForm).then(async (resp) => {
            loginActive();
            
            if (resp.code !== 0) {
                this.$message.error(resp.codeMsg)
            }
            else {
                this.changeLogin({ Authorization: resp.data })
                const userResp = await userInfo()
